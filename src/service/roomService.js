...
const createRoomService = (roomName) => {
  if (!roomName) {
...
const renameRoomService = (currentRoomName, newRoomName) => {
...
// Видалення кімнати
const deleteRoomService = (roomName) => {
  if (!roomName) {
    throw ApiError.BadRequest('Room name is required.');
  }
...
const getRoomMessageService = (roomName) => {
  if (!rooms[roomName]) {
    throw ApiError.NotFound('Room not found.');
  }
